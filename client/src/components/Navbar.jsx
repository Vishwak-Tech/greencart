  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative">
      ...
      <div className="hidden lg:flex items-center gap-8">
        <NavLink
          to="/seller"
          className="font-bold border border-gray-300 rounded-md px-4 py-2"
        >
          Seller Dashboard
        </NavLink>
        ...
        {!user ? (
          ...
          <div className="relative group">
            ...
            <ul className="hidden group-hover:block absolute top-10 right-0 bg-white shadow
              border border-gray-200 py-2.5 w-32 rounded-md text-sm z-40"
            >
              <li onClick={() => navigate("my-orders")} className="p-1.5 pl-3 hover:bg-bg-primary/10 cursor-pointer">My Orders</li>
      ...
      {/* ================= Mobile Menu ================= */}
      {open && (
        <div className="absolute top-[64px] left-0 w-full bg-white shadow-md py-4 flex flex-col items-start gap-3 px-5 text-sm lg:hidden">
          <NavLink to="/" onClick={() => setOpen(false)}>
            Home
          </NavLink>
          ...
          {!user ? (
            <button
              ...
              className="mt-2 px-6 py-2 bg-primary text-white rounded-full text-sm"
            >
              Login
            </button>
          ) : (
